...
def add_segmentations(df, image_dir="", testing=False, cache_path=None, only_cache=False):
    ...
    for _, row in df.iterrows():
        ...
        if cache_path and 'path' in cache_df.columns:
            ...
            if not cached_row.empty and isinstance(cached_row.iloc[0]['segmentation'], (list, tuple, np.ndarray)) and pd.notnull(cached_row.iloc[0]['segmentation'].all() or pd.notnull(cached_row.iloc[0]['segmentation'])):
                # Use cached data
