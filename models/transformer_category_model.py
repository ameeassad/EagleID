...
class TransformerCategory(pl.LightningModule):
    ...
    def __init__(self, 
        ...
        # Comprehensive metrics for 5-class classification - use compute_on_step=False for proper epoch-level metrics
        self.train_acc = Accuracy(task='multiclass', num_classes=self.num_classes, compute_on_step=False)
        self.val_acc = Accuracy(task='multiclass', num_classes=self.num_classes, compute_on_step=False)
        self.top2_acc = Accuracy(task='multiclass', num_classes=self.num_classes, top_k=2, compute_on_step=False)
        
        # Per-class metrics - use compute_on_step=False for proper epoch-level metrics
        self.val_precision = Precision(task="multiclass", num_classes=self.num_classes, average="macro", compute_on_step=False)
        self.val_recall = Recall(task="multiclass", num_classes=self.num_classes, average="macro", compute_on_step=False)
        self.val_f1 = F1Score(task="multiclass", num_classes=self.num_classes, average="macro", compute_on_step=False)
